...
class MPC:
    ...
    def set_cost(self, z0, A, B, C, q=1, qh=1, r=0.1):
        ...
        if self.Xub_soft is not None:
            self.H = sci_la.block_diag(self.H,0*np.eye(self.nslake))
            self.f = np.hstack([self.f, 1e3*np.ones((1, self.nslake))])
    ...
    def set_constraints(self, z0, A, B, C):
        ...
        if self.Xub_soft is not None:
            self.L = np.hstack([Au, np.zeros((np.size(Au,0),self.nslake))])
        else:
            self.L = Au
        self.w = bu
        ...
        P = np.kron(np.eye(self.nh),P_unit)
        ...
        if self.Xub_soft is not None:
            Aieq1 = self.L
            Aieq2 = np.hstack([Az_U, np.vstack([-np.eye(self.nslake),-np.eye(self.nslake)])])
            Aieq3 = np.hstack([np.zeros((self.nslake,self.m*self.nh)), -np.eye(self.nslake) ])
            self.L = np.vstack([Aieq1, Aieq2, Aieq3])
            bieq1 = self.w
            bsoft_unit = np.vstack([self.Xub_soft,-1*self.Xlb_soft])
            bsoft = np.kron(np.ones((self.nh,1)),bsoft_unit)
            bieq2 = bsoft - np.matmul(Az_z0,z0)
            bieq3 = np.zeros((self.nslake,1))
