...
while not done:
    if  t <= n0:
        ...
        if t == n0:
            ...
            metric = np.asarray(env_equalfilling.data_log["flow"]["001"]).reshape(n0+1,ncost)
            ...
            actions_mpc = KMPC.getMPC(z0,ulast_scaled,A,B,C,Mr)
        ...
        metric = np.asarray(env_equalfilling.data_log["flow"]["001"])
...
plt.plot(env_equalfilling.data_log["flow"]["001"][n0:])
plt.axhline(Mr, color="r",label="Reference"+str(Mr))
plt.ylabel('Outflows')
