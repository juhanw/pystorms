...
class denseMPC:
    ...
    def __init__(self,A, B, C, Q = None, R = None, Qh = None, ulin = None, qlin = None, 
        ...
        if np.size(Xlb) == self.n:
            self.Az = np.hstack((-np.eye(5),np.zeros((5,4)) )) #-C
            self.bz = -1*Xlb
        else:
            self.Az = np.zeros((self.n,self.nk))
            self.bz = np.zeros((self.n,1))
            
        if np.size(Xub) == self.n:
            self.Az = np.vstack((self.Az,np.hstack((np.eye(5),np.zeros((5,4)))) ))  #np.vstack((self.Az,C))  
            self.bz = np.vstack((self.bz,Xub))
        else:
            self.Az = np.vstack((self.Az,np.zeros((self.n,self.nk)) ))
            self.bz = np.vstack((self.bz,np.zeros((self.n,1))))
    ...
    def getMPC(self, z0,A,B,C,xr = None):
        ...
        Eineq = np.zeros((np.shape(Gineq)[0],self.nk))
        Eineq[:np.shape(self.Az)[0],:] = -1*self.Az
        wineq_z = np.tile(self.bz.reshape(2*self.n,1),(self.nh,1))
        wineq_u = np.tile(self.bu.reshape(2*self.m,1),(self.nh,1))
        wineq = np.vstack((wineq_z,wineq_u))
        ...
        try:
            ...
            u0 = np.asarray(sol['x'][self.nh*self.nk + self.m : self.nh*self.nk + 2*self.m])
        except:
            print("Error Constraints!") # why there is no Ulb/Uub effect???
            u0 = np.zeros((self.m,1))
